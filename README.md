...
[![Build Status](https://github.com/akiomik/vitest-mock-socket/actions/workflows/ci.yml/badge.svg)](https://github.com/akiomik/vitest-mock-socket/actions)
[![codecov](https://codecov.io/gh/akiomik/vitest-mock-socket/branch/main/graph/badge.svg?token=40OVYIT90L)](https://codecov.io/gh/akiomik/vitest-mock-socket) -->

A set of utilities and Vitest matchers to help testing complex websocket interactions.
A patched fork of [vitest-mock-socket](https://github.com/andrewangelle/vitest-mock-socket).
...
`vitest-mock-socket` registers custom vitest matchers to make assertions
...
`vitest-mock-socket` can also automatically serialize and deserialize
JSON messages:
...
When testing React applications, `vitest-mock-socket` will look for
...
`vitest-mock-socket` uses [Mock Socket](https://github.com/thoov/mock-socket)
...
the `mock-socket` library that `vitest-mock-socket` uses under the hood only
implements the browser API.
As a result, `vitest-mock-socket` will only work with the `ws` library if you
